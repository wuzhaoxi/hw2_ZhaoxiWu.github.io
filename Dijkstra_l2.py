...
lines=[line1,line2,line3,line4,line5,line6,line7,line8,line9,line10,line11,line12,line13,line16]
delays=[delay1,delay2,delay3,delay4,delay5,delay6,delay7,delay8,delay9,delay10,delay11,delay12,delay13,delay16]
#######################################
...
def find_delay(k, j):
  for z in range(len(lines)):
    for i in range(len(lines[z])):
      if lines[z][i] == k:
        if i < len(lines[z])-1 and lines[z][i+1] == j:
          return delays[z][i]
  return -1
...
def _calc_paths ():
  ...
  for k in sws:
    for j,port in adjacency[k].iteritems():
     ...
     # path_map[k][j] = (find_delay(k,j),None)
      path_map[k][j] = (1,None)
  ...
  for k in sws:
    check_over = []
    node = k
    while len(check_over) < len(sws):
      check_over.append(node)
      for j in sws:
        if path_map[k][node][0] is not None and path_map[node][j][0] is not None:
          if j not in check_over:
            kj_dist = path_map[k][node][0] + path_map[node][j][0]
            if path_map[k][j][0] is None or kj_dist < path_map[k][j][0]:
              path_map[k][j] = (kj_dist, node)
            node=j
